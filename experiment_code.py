...
# Windows DPI Awareness - only on Windows and if not in PyInstaller
...
# Detect available screens and use primary screen if secondary not available
SCREEN_INDEX = 1
try:
    # Try to get number of screens
    import tkinter as tk
    root = tk.Tk()
    screen_count = root.winfo_screenwidth()  # This will fail if no display
    root.destroy()
    
    # If we have multiple screens, try to use the second one
    if pyglet.canvas.get_display().get_screens():
    ...
    SCREEN_INDEX = 0
    pylog.warning("Using primary screen (index 0)")

mon = monitors.Monitor("scr")
mon.setSizePix((1920, 1080))
mon.save()

# Create a window on the specified screen with error handling
try:
    win = visual.Window(
        size=(1920, 1080),
        fullscr=True,
    ...
    pylog.warning(f"Failed to create window on screen {SCREEN_INDEX}, trying primary screen: {e}")
    try:
        win = visual.Window(
            size=(1920, 1080),
            fullscr=True,
        ...
        pylog.error(f"Failed to create window on any screen: {e2}")
        # Try windowed mode as last resort
        win = visual.Window(
            size=(1920, 1080),
            fullscr=False,
            monitor=mon,
...
def perform_calibration():
    ...
    longest = max(db, key=lambda r: sum(len(w["word"]) for w in r["word_centroids"]))
    words_plain = [w["word"] for w in longest["word_centroids"]]
    ...
    for nx, ny in defaults:
        ...
        for wi, wdat in enumerate(longest["word_centroids"]):
...
# ─────────────────────────────────────────────────────────────────────────────
if not ABORT:
    fx_rel = db[0]["word_centroids"][0]["centroids"][0]
